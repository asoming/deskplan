...
(() => {
  ...
  function setTransparency(value) { document.body.classList.toggle('clear-desktop', value >= 70); document.documentElement.style.setProperty('--alpha', String((100 - value) / 100)); $('#opacity-label').textContent = `${value}%`; $('#transparency').value = value; $('#transparency-value').textContent = `${value}%`; }
