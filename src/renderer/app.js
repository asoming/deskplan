...
(() => {
  ...
  function error(error) { toast(error.message || '操作未完成，请重试'); $('#save-status').textContent = '操作未完成'; }
  function action(fn) { return event => Promise.resolve().then(() => fn(event)).catch(error); }
  async function call(name, payload) { const result = await api.call(name, payload); $('#save-status').textContent = '本地保存'; return result; }
  ...
  function receive(next) {
    ...
    $('#calendar-toggle').setAttribute('aria-label', state.settings.calendarOpen ? '收起日历' : '显示日历');
    $('#pin-toggle').textContent = state.settings.alwaysOnTop ? '✓ 始终置顶' : '始终置顶';
  ...
  function renderBoard() {
    ...
    $('#board').innerHTML = labels.map((label, level) => {
      ...
      return `<section class="zone" data-level="${level}" aria-label="${label}"><div class="zone-heading"><h2>${label}<span class="count">${list.length}</span></h2><button data-add="${level}" aria-label="在${label}添加任务">${icon('plus')}</button></div><div class="task-list">${list.map(taskHTML).join('') || '<div class="empty-zone">拖入文件，或点 ＋ 添加</div>'}</div></section>`;
    }).join('');
    document.querySelectorAll('.task-list').forEach((list, i) => { list.scrollTop = scroll[i] || 0; });
    $('#task-count').textContent = `${active.length} 件待办`;
  }
  function taskHTML(t) {
  ...
  function renderPlanner() {
    ...
    $('#compact-toggle').textContent = compactMode ? '展开' : '小窗';
    $('#compact-toggle').setAttribute('aria-label', compactMode ? '展开完整计划' : '缩为置顶小窗');
    ...
    $('#task-count').textContent = `${active.length} 件待办`;
    $('#view-caption').textContent = view === 'quadrants' ? '按紧急程度，轻重有序' : view === 'week' ? '安排日期与截止日期独立' : view === 'inbox' ? '先记下来，稍后安排' : new Date(now).toLocaleDateString('zh-CN', { month: 'long', day: 'numeric', weekday: 'long' });
    const total = todays.reduce((sum, t) => sum + t.estimatedMinutes, 0);
    if (view === 'today') {
      $('#planner').className = 'planner today-view';
      $('#planner').innerHTML = `<div class="view-heading"><div><h1>把今天，安排好。</h1><p>${todays.length} 件待办 · 已估时 ${total} 分钟${todays.some(t => !t.estimatedMinutes) ? ' · 部分任务未估时' : ''}</p></div><button class="text-button" data-add-today>＋ 安排一件事</button></div><section class="today-focus" data-focus-drop="true"><div class="section-heading"><h2>最重要的三件事</h2><span>${top.length} / 3</span></div><div class="focus-grid">${top.map(t => planRow(t, true)).join('')}${Array.from({ length: 3 - top.length }, (_, i) => `<div class="focus-placeholder"><span>0${top.length + i + 1}</span><small>点 ☆ 或拖入重要任务</small></div>`).join('')}</div></section><section class="today-others" data-focus-drop="false"><div class="section-heading"><h2>其他待办</h2><span>拖动排序</span></div><div class="today-list">${other.map(t => planRow(t)).join('') || '<div class="empty-zone">留一点空白给自己。</div>'}</div></section>`;
    } else if (view === 'week') {
      const d = new Date(now); d.setDate(d.getDate() + weekOffset * 7); const days = weekDays(d);
      $('#planner').className = 'planner week-view';
      $('#planner').innerHTML = `<div class="view-heading"><div><h1>一周的节奏</h1><p>${days[0].slice(5)} — ${days[6].slice(5)} · 每日容量 ${state.settings.dailyCapacity} 分钟</p></div><div><button data-week="-1" aria-label="上一周">${icon('left')}</button><button data-week="0">本周</button><button data-week="1" aria-label="下一周">${icon('right')}</button></div></div><div class="week-board">${days.map((day, i) => {
        ...
        return `<section class="week-lane" data-schedule="${day}" data-today="${day === today}"><div class="week-title"><span>周${'一二三四五六日'[i]}</span><strong>${Number(day.slice(-2))}</strong><button data-add-day="${day}" aria-label="安排到 ${day}">＋</button></div><div class="workload ${minutes > state.settings.dailyCapacity ? 'overloaded' : ''}"><span>${minutes} 分钟${minutes > state.settings.dailyCapacity ? ' · 偏满' : ''}</span><div class="load-track"><i style="width:${Math.min(100, minutes / state.settings.dailyCapacity * 100)}%"></i></div><small>${unestimated ? `${unestimated} 件未估时` : `${list.length} 件任务`}</small></div><div class="week-tasks">${list.map(t => planRow(t, t.focusDay === today)).join('') || '<div class="empty-zone">拖入安排</div>'}</div></section>`;
      }).join('')}</div>`;
    } else if (view === 'inbox') {
      $('#planner').className = 'planner inbox-view';
      $('#planner').innerHTML = `<div class="view-heading"><div><h1>让想法先落下来。</h1><p>无需选日期。准备好时，再安排进计划。</p></div><button id="capture-inbox">随手记 ＋</button></div><section data-inbox-drop class="inbox-list">${inbox.map(t => planRow(t)).join('') || '<div class="inbox-empty"><span>收集箱是空的</span><small>Ctrl / ⌘ + Shift + 空格，随时记下一件事</small></div>'}</section>`;
    }
    if (compactMode) {
      ...
      $('#compact-panel').innerHTML = unique.length ? unique.map((t, i) => `<div class="compact-task ${i === 0 ? 'current-task' : ''}"><small>${i === 0 ? '当前任务' : '接下来'}</small><div class="task-top"><input type="checkbox" data-complete="${esc(t.id)}" aria-label="完成：${esc(t.title)}"><button class="task-title" data-expand-edit="${esc(t.id)}">${esc(t.title)}</button><small>${t.estimatedMinutes ? `${t.estimatedMinutes} 分` : ''}</small></div></div>`).join('') : '<div class="empty-zone">今天留一点空白。<br>点 ＋，安排一件事。</div>';
    }
  }
  ...
  document.addEventListener('click', action(async e => {
    ...
    const star = e.target.closest('[data-focus]'); if (star) { const t = state.tasks.find(t => t.id === star.dataset.focus); await call('plan', { id: t.id, day: dateKey(new Date(now)), focus: t.focusDay !== dateKey(new Date(now)) }); toast(t.focusDay ? '已移出今日重点' : '已加入今日重点', true); }
    const today = e.target.closest('[data-today]'); if (today && today.tagName === 'BUTTON' && today.dataset.today !== 'true' && today.dataset.today !== 'false') { await call('plan', { id: today.dataset.today, day: dateKey(new Date(now)) }); toast('已安排今天', true); }
    const start = e.target.closest('[data-start]'); if (start) { await call('current', { id: start.dataset.start }); toast('已设为当前任务，小窗中可继续查看', true); }
  ...
  function renderCalendar() {
    ...
    for (let d = 1; d <= total; d++) {
      ...
      days += `<button class="day" data-day="${key}" aria-pressed="${selectedDay === key}" data-today="${key === dateKey(new Date(now))}" aria-label="${key}，${count} 件到期任务">${d}${count ? '<span class="day-dot"></span>' : ''}</button>`;
    }
    const day = selectedDay || dateKey(new Date(now)), list = active.filter(t => taskDay(t) === day).sort((a, b) => a.due.localeCompare(b.due));
    $('#calendar').innerHTML = `<div class="month-heading"><span>${y} 年 ${m + 1} 月</span><div class="month-navigation"><button data-month="-1" aria-label="上个月">${icon('left')}</button><button id="calendar-today" class="today-button">今天</button><button data-month="1" aria-label="下个月">${icon('right')}</button></div></div><div class="weekdays">${['一', '二', '三', '四', '五', '六', '日'].map(s => `<span>${s}</span>`).join('')}</div><div class="calendar-days">${days}</div><div class="agenda"><div class="agenda-header"><span>${day.slice(5).replace('-', ' / ')} 到期</span><button id="calendar-add" aria-label="在选中日期新建任务">${icon('plus')}</button></div>${list.map(t => `<button class="agenda-item" data-edit="${esc(t.id)}"><small>${localTime(t.due).slice(11)}</small><span>${esc(t.title)}</span></button>`).join('') || '<div class="agenda-empty">没有到期任务</div>'}</div><p class="calendar-hint">截止日历 · 拖入修改截止日期</p>`;
  }

  function ask(title, detail, options = [{ label: '取消', value: false }, { label: '确定', value: true, primary: true }]) {
  ...
  async function closeEditor() {
    ...
    if (editorSignature() !== editorInitial) {
      const choice = await ask('保存这次编辑？', '还没有保存的修改将会丢失。', [{ label: '继续编辑', value: false }, { label: '放弃修改', value: 'discard' }, { label: '保存', value: 'save', primary: true }]);
  ...
  async function saveEditor() {
    ...
    if (!value.title) { $('#task-title').setCustomValidity('请填写任务名称'); $('#task-title').reportValidity(); return false; }
    if (value.due && Date.parse(value.due) < Date.now() && value.due !== previous?.due) {
      if (!await ask('这个截止时间已经过去', '保存后，任务会显示为已逾期。仍然保存吗？')) return false;
    }
    ...
    try {
      ...
      toast(value.level === 1 && effectiveLevel({ ...value, status: 'active' }, Date.now()) === 0 ? '已保存，距截止不超过 48 小时，显示在「马上做」' : '已保存', true);
      return true;
  ...
  function renderAttachments() {
    ...
    $('#attachments').innerHTML = files.length ? files.map((file, index) => {
      ...
      return `<div class="attachment ${missing ? 'missing' : ''}"><button type="button" class="attachment-name" data-file-open="${esc(file.id || '')}" title="${esc(file.path)}" ${!editingId ? 'disabled' : ''}>${icon('file')}<span>${esc(file.name)}${missing ? ' · 文件不可用' : ''}</span></button><div class="attachment-actions">${editingId ? `<button type="button" data-relink="${esc(file.id)}" aria-label="重新定位 ${esc(file.name)}">重定位</button><button type="button" data-folder="${esc(file.id)}" aria-label="打开 ${esc(file.name)} 所在文件夹">${icon('folder')}</button>` : ''}<button type="button" data-file-remove="${esc(file.id || index)}" aria-label="移除 ${esc(file.name)} 的关联">${icon('close')}</button></div></div>`;
    }).join('') : '<div class="empty-attachments">添加文件后，可以在这里直接打开原文件。</div>';
  }
  function openLibrary(filter = 'all') {
    closePopovers(); $('#library-filter').value = filter; $('#search-input').value = '';
    $('#library-title').textContent = filter === 'completed' ? '已完成' : filter === 'deleted' ? '回收站' : '搜索任务';
  ...
  function renderLibrary() {
    ...
    $('#library-list').innerHTML = tasks.length ? tasks.map(t => `<div class="library-item"><button class="library-title" data-edit="${esc(t.id)}"><span>${esc(t.title)}</span><small>${t.status === 'active' ? labels[effectiveLevel(t, now)] + ' · ' + fullDate(t.due) : t.status === 'completed' ? '完成于 ' + fullDate(t.completedAt) : t.status === 'cancelled' ? '已取消 · 可恢复' : '删除于 ' + fullDate(t.deletedAt)}</small></button><div class="library-actions">${['completed', 'cancelled'].includes(t.status) ? `<button data-status="reopen" data-id="${esc(t.id)}">恢复待办</button>` : ''}${t.status === 'deleted' ? `<button data-status="restore" data-id="${esc(t.id)}">恢复</button><button class="danger-text" data-purge="${esc(t.id)}">永久删除</button>` : `<button class="danger-text" data-status="delete" data-id="${esc(t.id)}" aria-label="删除 ${esc(t.title)}">${icon('trash')}</button>`}</div></div>`).join('') : '<div class="library-empty">这里还没有任务</div>';
  }
  function renderSettings() {
    ...
    $('#quick-hint').textContent = !state.settings.quickCapture ? '已关闭' : state.native.quickShortcutRegistered ? '输入后回车保存到收集箱' : '快捷键被占用或系统不支持，请换一组或用托盘随手记';
  ...
  async function reschedule(id, targetDay) {
    ...
    if (target.getTime() < Date.now() && !await ask('改到过去的日期？', `截止时间将改为 ${label}，任务会标记为逾期。`)) return;
    await call('update', { id, patch: { due: target.toISOString() } }); selectedDay = targetDay; renderBoard(); renderCalendar(); toast(`已改为 ${label}`, true);
  }
  ...
  $('#undo').onclick = action(async () => { await call('undo'); toast('已撤销'); });
  $('#export-backup').onclick = action(async () => { if (await call('backup:export')) toast('备份已导出，不包含原文件'); });
  $('#restore-backup').onclick = action(async () => { if (await call('backup:restore')) { selectedDay = ''; renderBoard(); renderCalendar(); toast('已从备份恢复'); } });
  ...
  $('#delete-task').onclick = action(async () => { await call('status', { id: editingId, action: 'delete' }); $('#task-dialog').close(); editingId = null; toast('已移到回收站，原文件保持不变', true); });
  $('#complete-task').onclick = action(async () => { const id = editingId; if (editorSignature() !== editorInitial && !await saveEditor()) return; await call('status', { id, action: 'complete' }); $('#task-dialog').close(); editingId = null; toast('已完成', true); });
  ...
  document.addEventListener('click', action(async event => {
    ...
    const status = event.target.closest('[data-status]'); if (status) { await call('status', { id: status.dataset.id, action: status.dataset.status }); toast(status.dataset.status === 'delete' ? '已移到回收站' : '任务已恢复', true); }
    const purge = event.target.closest('[data-purge]'); if (purge && await call('purge', { id: purge.dataset.purge })) toast('已永久删除任务记录');
  ...
  document.addEventListener('change', action(async event => { if (!event.target.matches('[data-complete]')) return; const input = event.target; try { await call('status', { id: input.dataset.complete, action: 'complete' }); toast('已完成', true); } catch (e) { input.checked = false; throw e; } }));
  ...
  const handleDrop = action(async event => {
    ...
    if (event.dataTransfer.files.length && onTask && !zone) { await api.dropFiles(Array.from(event.dataTransfer.files), { taskId: onTask.dataset.task }); toast('已关联文件', true); return; }
    if (id && (schedule || focus)) {
      await call('plan', { id, day: schedule?.dataset.schedule || dateKey(new Date(now)), ...(focus ? { focus: focus.dataset.focusDrop === 'true' } : {}), beforeId: onTask?.dataset.task === id ? undefined : onTask?.dataset.task }); toast('已调整安排，截止时间保持不变', true); return;
    }
    if (id && inbox) { await call('update', { id, patch: { inbox: true } }); toast('已放回收集箱', true); return; }
    if (event.dataTransfer.files.length && zone) {
      const task = event.target.closest('[data-task]');
      await api.dropFiles(Array.from(event.dataTransfer.files), { taskId: task?.dataset.task, level: Number(zone.dataset.level) }); toast(task ? '已关联文件' : '已从文件创建任务', true); return;
    }
    if (id && date) await reschedule(id, date.dataset.day);
      ...
      const task = state.tasks.find(t => t.id === id); toast(task && level === 1 && effectiveLevel(task, Date.now()) === 0 ? '距截止不超过 48 小时，仍显示在「马上做」' : `已移到「${labels[level]}」`, true);
    }
  });
  ...
  document.addEventListener('keydown', action(async event => {
    ...
    if (event.key.toLowerCase() === 'z' && !document.querySelector('dialog[open]') && state.canUndo) { event.preventDefault(); await call('undo'); toast('已撤销'); }
  }));

  if (!api) { $('#task-count').textContent = '请通过桌面应用启动日序'; return; }
  api.onState(receive);
  ...
  call('state').then(data => { receive(data); if (data.recoveryNotice) toast(data.recoveryNotice); else {
    ...
    if (overdue) toast(`${overdue} 件任务已逾期，打开任务可调整日期`);
  } }).catch(error);
})();
