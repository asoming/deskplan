...
module.exports=async function checkWindow(runtime,js,call){
 ...
 assert.equal(await js('getComputedStyle(document.querySelector(".toolbar")).webkitAppRegion'),'no-drag');
 ...
 assert.equal(await js('getComputedStyle(document.querySelector(".toolbar")).webkitAppRegion'),'drag');
