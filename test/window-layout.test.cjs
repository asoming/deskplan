...
test('top right docking respects work area, reserved space and negative monitor origins',()=>{
 ...
 assert.deepEqual(panelBounds(DEFAULT_SETTINGS,area,area),{x:-784,y:64,width:760,height:540});
 const inset=panelBounds({...DEFAULT_SETTINGS,desktopInset:120},area,area);assert.equal(inset.x,-904);
 const small=panelBounds({...DEFAULT_SETTINGS,compactMode:true},area,area);assert.equal(small.x,-414);assert.equal(small.y,64);
});
