...
`npm run test:release` exercises desktop blend mode, checklist editing, recurrence generation/undo, removal of mouse-through and window-close buttons, two-button mini mode, context menus, real unlocked pointer dragging, and GitHub update checks and independent transparent surfaces/readable text. It also tests right-corner docking, reserved desktop space, a fixed yet interactive panel, manual position preservation, both panels staying non-topmost, no focus stealing during a background check, and accessible translated sidebar icons without a panel logo. `npm run test:planner` additionally covers Today reordering, the three-priority limit, the seven-day workload view, quick capture and mini-window behavior.
